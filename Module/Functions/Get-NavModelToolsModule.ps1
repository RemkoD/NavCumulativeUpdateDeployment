...
function Get-NavModelToolsModule
{
    ...
    Process
    {
        $ModuleName = 'Microsoft.Dynamics.Nav.Model.Tools'

        $NavRTC = Get-NavComponent -NavVersion $NavVersion | Where-Object Component -eq 'RTC'

        if (-not $NavRTC.IsInstalled) {
            
            return $false
        }

        $ModulePath = Join-Path -Path $NavRTC.InstallLocation -ChildPath $($ModuleName + ".psd1")
...
Export-ModuleMember -Function Get-NavModelToolsModule
